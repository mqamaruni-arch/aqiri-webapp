(() => {
  ...
  if (results) {
    ...
    const render = () => {
      ...
      const matches = books.filter((book) => [book.title, book.subtitle, book.author, book.description, book.publisher, book.category, book.language, book.year].filter(Boolean).join(" ").toLocaleLowerCase().includes(query)).sort((a, b) => a.title.localeCompare(b.title));
  ...
  const fields = [["Author / editor", book.author], ["Publisher", book.publisher], ["Publication year", book.year], ["Subject", book.category], ["Language", book.language], ["Edition", book.edition], ["ISBN", book.isbn], ["DOI", book.doi], ["Pages", book.pages], ["Open-access status", book.access], ["Original source", book.sourceUrl], ["Suggested citation", book.citation]].filter(([, value]) => value);
  const cover = book.cover || (book.generatedCover ? "" : `${book.file}-title-page.png`);
  const pdf = book.pdf || `${book.file}.pdf`;
  const description = book.description && book.description.trim().length >= 80 ? book.description : "";
  target.innerHTML = `<nav class="simple-breadcrumb" aria-label="Breadcrumb"><a href="index.html">Home</a><span>/</span><a href="library.html">Library</a><span>/</span><span aria-current="page">Record</span></nav><article class="book-record">${cover ? `<div class="book-record-cover"><img src="assets/library/${esc(cover)}" alt="Cover of ${esc(book.title)}" onerror="this.parentElement.remove()"></div>` : ""}<div class="book-record-content"><p class="open-label">AQIRI Library</p><h1>${esc(book.title)}</h1>${book.subtitle ? `<p class="book-record-subtitle">${esc(book.subtitle)}</p>` : ""}<p class="book-record-author">${esc(book.author || "Author information unavailable")}${book.year ? ` · ${esc(book.year)}` : ""}</p>${description ? `<div class="book-record-description"><h2>Description</h2><p>${esc(description)}</p></div>` : ""}<dl class="book-record-details">${fields.map(([label, value]) => `<div><dt>${esc(label)}</dt><dd>${label === "DOI" ? `<a href="https://doi.org/${encodeURIComponent(value)}" target="_blank" rel="noopener">${esc(value)} ↗</a>` : label === "Original source" ? `<a href="${esc(value)}" target="_blank" rel="noopener">View publisher record ↗</a>` : esc(value)}</dd></div>`).join("")}</dl><section class="book-record-access"><h2>Publication access</h2><div class="book-record-actions"><a class="button button-green" href="assets/library/${esc(pdf)}" target="_blank" rel="noopener">Read Publication <span>↗</span></a><a class="download-pdf" href="assets/library/${esc(pdf)}" download="${esc(pdf)}">Download Publication <span>↓</span></a></div></section></div></article>`;
})();
