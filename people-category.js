(() => {
  ...
  const filters = {
    // The fellows directory is deliberately limited to profiles
  ...
  const researchOrder = ["shadeka-jannat", "emiola-habeeb", "dr-ade-jamarudin", "dr-idris-abdur-rahman", "ibraheem-abdulwahab-abdulkareem", "abba-a-dandago"];
  if (category === "research") {
    people.sort((a, b) => researchOrder.indexOf(a[0]) - researchOrder.indexOf(b[0]));
  }
  root.innerHTML = people.length ? people.map(([slug, name, role, title, affiliation, photo]) => `<article class="staff-card" data-person="${slug}">${photo ? `<img class="staff-photo" src="${photo}" alt="${name}">` : ""}<span>${role}</span><h4>${name}</h4><p>${title}<br>${affiliation}</p><a href="person-profile.html?person=${encodeURIComponent(slug)}">View full profile <b>→</b></a></article>`).join("") : '<p class="directory-empty">Current appointments will be announced by AQIRI.</p>';
})();
